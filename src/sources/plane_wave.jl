...
struct PlaneWaveSource{T<:AbstractFloat} <: AbstractCurrentSource{T}
    ...
    function PlaneWaveSource{T}(
        ...
        abs(k_norm - 1) < 1e-10 || throw(ArgumentError("k_vector must be normalized (|k| = 1)"))
        
        # Check transverse field condition (E ⟂ k)
        E_dot_k = real(dot(polarization, k_vector))
        abs(E_dot_k) < 1e-10 || throw(ArgumentError("polarization must be transverse to k_vector"))
...
function _generate_incident_fields_arrays(
    ...
    for component in 1:3
        E_field[:, :, :, component] = source.amplitude * source.polarization[component] * phase_array
        H_field[:, :, :, component] = source.amplitude * H_polarization[component] * phase_array
    end
