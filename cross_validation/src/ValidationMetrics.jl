    ...
    if haskey(matlab_results, "E_field") && haskey(reference_results, "E_field")
        ...
        if size(E_matlab) == size(E_ref)
            ...
            if ref_magnitude > 1e-15
                relative_errors = field_diff ./ (field_magnitude_ref .+ 1e-15 * ref_magnitude)
...
function compute_convergence_metrics(matlab_results::Dict)
    ...
    if haskey(matlab_results, "convergence_history")
        ...
        if !isempty(conv_hist) && length(conv_hist) > 1
            ...
            metrics["iterations_to_convergence"] = converged_idx !== nothing ? converged_idx : n_points
        ...
        if size(E_matlab) == size(E_ref)
            ...
            if !isempty(errors) && all(isfinite.(errors))
                ...
                if sum(significant_mask) > 0
                    rel_errors = errors[significant_mask] ./ ref_magnitudes[significant_mask]
...
function analyze_convergence(convergence_history::Vector{Float64})
    ...
    if n > 1
        ...
        if n > 3
            ...
            recent_change = abs(convergence_history[end] - convergence_history[end-recent_fraction+1])
...
function check_tolerances(metrics::Dict, tolerances::Dict)
    try
        ...
        if haskey(tolerances, "field_relative_error") && haskey(metrics, "field_relative_error")
        ...
        if haskey(tolerances, "energy_relative_error") && haskey(metrics, "energy_relative_error")
        ...
        if haskey(tolerances, "convergence_threshold") && haskey(metrics, "convergence_achieved")
...
function convert_metrics_to_dict(metrics::ValidationMetricsResult)
    ...
    # Field validation metrics
    result["field_absolute_error"] = isnan(metrics.field_absolute_error) ? 0.0 : metrics.field_absolute_error
    result["field_relative_error"] = isnan(metrics.field_relative_error) ? 0.0 : metrics.field_relative_error
    result["field_rms_error"] = isnan(metrics.field_rms_error) ? 0.0 : metrics.field_rms_error
    result["field_max_error"] = isnan(metrics.field_max_error) ? 0.0 : metrics.field_max_error
    result["field_mean_error"] = isnan(metrics.field_mean_error) ? 0.0 : metrics.field_mean_error
    result["field_correlation"] = isnan(metrics.field_correlation) ? 0.0 : metrics.field_correlation
    
    # Energy metrics
    result["energy_relative_error"] = isnan(metrics.energy_relative_error) ? 0.0 : metrics.energy_relative_error
    result["energy_absolute_error"] = isnan(metrics.energy_absolute_error) ? 0.0 : metrics.energy_absolute_error
    result["energy_conservation_ratio"] = isnan(metrics.energy_conservation_ratio) ? 1.0 : metrics.energy_conservation_ratio
    
    # Convergence metrics
    result["final_residual"] = isnan(metrics.final_residual) ? 1.0 : metrics.final_residual
    result["convergence_rate"] = isnan(metrics.convergence_rate) ? 0.0 : metrics.convergence_rate
    result["iterations_to_convergence"] = Float64(metrics.iterations_to_convergence)
    
    # Statistical metrics
    result["relative_error_std"] = isnan(metrics.relative_error_std) ? 0.0 : metrics.relative_error_std
    result["relative_error_mean"] = isnan(metrics.relative_error_mean) ? 0.0 : metrics.relative_error_mean
    result["outlier_fraction"] = isnan(metrics.outlier_fraction) ? 0.0 : metrics.outlier_fraction
    
    # Quality indicators
    result["numerical_stability"] = isnan(metrics.numerical_stability) ? 0.0 : metrics.numerical_stability
    result["solution_quality"] = isnan(metrics.solution_quality) ? 0.0 : metrics.solution_quality
    result["validation_confidence"] = isnan(metrics.validation_confidence) ? 0.0 : metrics.validation_confidence
