...
@testset "LinearSolve.jl Performance Benchmarks" begin
    @testset "Small Scale Performance (64×64×32)" begin
        ...
        # Benchmark BiCGStab  
        bench_bicgstab, E_bicgstab,
        H_bicgstab = benchmark_solver(config_bicgstab, permittivity, source)
    ...
    @testset "Medium Scale Performance (128×128×64)" begin
        ...
        # Benchmark iterative method (baseline)
        bench_iter, E_iter,
        H_iter = benchmark_solver(config_iter, permittivity, source; samples = 2)
        ...
        # Benchmark GMRES
        bench_gmres, E_gmres,
        H_gmres = benchmark_solver(config_gmres, permittivity, source; samples = 2)
        gmres_time = median(bench_gmres.times) / 1e9
        ...
        # Benchmark BiCGStab
        bench_bicgstab, E_bicgstab,
        H_bicgstab = benchmark_solver(config_bicgstab, permittivity, source; samples = 2)
        bicgstab_time = median(bench_bicgstab.times) / 1e9
